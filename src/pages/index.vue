  ...
  <v-container>
    <v-row justify="center">
      <v-col cols="12">
        ...
        />
        <!-- @update:model-value="page = 1" 讓換選擇分類時，頁數回到 1 -->
        <v-chip-group v-model="selectedCategory" mandatory @update:model-value="page = 1">
          <v-chip filter text="全部" :value="''" variant="outlined" />
          <v-chip
          ...
          <v-menu>
            <template #activator="{ props }">
              <v-btn v-bind="props" append-icon="mdi-chevron-down" :ripple="false" variant="text">
      ...
      <v-col v-for="product in currentPageProducts" :key="product._id" cols="12" lg="4" md="6">
